...
func TestWithinTimeWindow(t *testing.T) {
	...
	candleTime := time.Unix(candle.Time, 0)

	// Case 1: Is within the time window
	ok, err := WithinHours(candleTime, "8:30", "20:30")
	assert.Nil(t, err)
	assert.True(t, ok)

	// Case 2: Is within the time window
	ok, err = WithinHours(candleTime, "12:08", "12:09")
	assert.Nil(t, err)
	assert.True(t, ok)

	// Case 3: Is NOT within the time window
	ok, err = WithinHours(candleTime, "5:08", "11:09")
	assert.Nil(t, err)
	assert.False(t, ok)

	// Case 4: Is NOT within the time window
	ok, err = WithinHours(candleTime, "12:01", "12:07")
	assert.Nil(t, err)
	assert.False(t, ok)

	// Case 5: Is NOT within the time window
	ok, err = WithinHours(candleTime, "12:09", "20:10")
	assert.Nil(t, err)
	assert.False(t, ok)

	// Case 6: Is within the time window
	ok, err = WithinHours(candleTime, "00:00", "23:59")
	assert.Nil(t, err)
	assert.True(t, ok)
}
